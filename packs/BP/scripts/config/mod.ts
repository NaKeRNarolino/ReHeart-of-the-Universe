...
export const config = {
  experimentalDDUI: new utils.WorldDynamicProperty<boolean>(
    "config:experimental_ddui",
    false,
  ),
};

const experimentalDDUIObservable = Observable.create<boolean>(false, {
  clientWritable: true,
});

export async function openConfig(player: mc.Player) {
