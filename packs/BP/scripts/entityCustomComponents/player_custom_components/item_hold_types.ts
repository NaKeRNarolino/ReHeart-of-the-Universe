...
import { EntityCustomComponent } from "../mod";
import { namespace } from "../../utils/namespace";
import {
  getSkintFocusColor,
  getSkintFocusOrbLocation,
} from "../../items/skint_card/custom_component";

const heldItemTypeId = namespace.namespaced("held_item_type");

export const ItemHoldComponent: EntityCustomComponent = {
  filter: {
    ...
    if (mainhand?.typeId == "naker_hotu:skint_magic_lens") {
      ...
      if (offhand?.typeId == "naker_hotu:skint_card") {
        ...
        player.dimension.spawnParticle(
          namespace.namespaced("skint_focus_idle_orb"),
          orb,
          mvm,
        );
      }
      if (heldItemType != "skint_lens") {
        player.playAnimation("animation.magic_lenses.player_held", {
          stopExpression:
            "!query.is_item_name_any('slot.weapon.mainhand', 'naker_hotu:skint_magic_lens')",
          blendOutTime: 0.1,
        });
