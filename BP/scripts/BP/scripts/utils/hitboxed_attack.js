...
export class HitboxedAttackArea {
     ...
     */
    attackUsing(player, damage, cause) {
        ...
        const hitboxMax = Math.max(Math.abs(this.start.x), Math.abs(this.start.y), Math.abs(this.start.z), Math.abs(this.end.x), Math.abs(this.end.y), Math.abs(this.end.z));
        ...
        for (const entity of entities) {
            if (entity.id != player.id) {
                const localPos = utils.absoluteToLocalCoordinates(player, entity.location);
                const isInside = localPos.x >= Math.min(this.start.x, this.end.x) &&
