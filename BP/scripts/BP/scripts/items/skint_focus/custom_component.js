...
import { getSkintFocusColor, getSkintFocusOrbLocation, } from "../skint_card/custom_component";
import { namespace } from "../../utils/namespace";
import { Vector3Utils } from "@minecraft/math";
export const SkintFocusCustomComponent = {
    onUse: (data) => {
        ...
        player.dimension.spawnParticle(namespace.namespaced("skint_focus_execute_orb"), orb, mvm);
        tryExecuteSpell(offhand, player);
    },
};
function tryExecuteSpell(card, player) {
    const rawSpell = card.getDynamicProperty(namespace.namespaced("skint_card_data"));
    ...
    try {
        ...
        for (const t of targets) {
            player.dimension.spawnParticle(namespace.namespaced("skint_focus_target_orb"), Vector3Utils.add(t.location, { x: 0, y: 1, z: 0 }), color.mvm);
        }
        if (action.type === "damage") {
            ...
            for (const t of targets) {
                ...
                const value = Math.min(health.currentValue + action.amount, health.effectiveMax);
...
function getSpellTargets(player, spell) {
    if (spell.data.target === "self") {
        ...
        const { dimension, x, y, z } = resolveLocation(player, spell.data.target.readFrom);
        console.warn(x, y, z);
...
function resolveLocation(player, location) {
    let loc = { x: 0, y: 0, z: 0 };
