...
export const HealthVialComponent = {
    onUse: async (data) => {
        const { itemStack: item, source: player } = data;
        if (player.getDynamicProperty(namespace.namespaced("use_health_vial")) == true) {
            return;
        }
        if (item == undefined)
            return;
        ...
        player.dimension.spawnParticle(namespace.namespaced("start_use_health_vial"), player.location, mvm);
        mvm.setFloat("variable.rotation_index", 0.5);
        player.dimension.spawnParticle(namespace.namespaced("start_use_health_vial"), player.location, mvm);
        await mc.system.waitTicks(60);
        player.dimension.spawnParticle(namespace.namespaced("use_health_vial"), player.location);
        ...
        const newItem = new mc.ItemStack(namespace.namespaced(`health_vial_${level - 1}`));
