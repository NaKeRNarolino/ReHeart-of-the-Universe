...
function tryIncrease(player) {
    const manager = StaminaManager.of(player);
    if (!manager.canIncrease())
        return;
    manager.increase(1);
}
function genericAttributes(player) {
    const manager = StaminaManager.of(player);
    if (manager.get() > 0)
        return;
...
mc.world.afterEvents.entityHitEntity.subscribe((data) => {
    ...
    if (damagingEntity.typeId == "minecraft:player") {
        const manager = StaminaManager.of(damagingEntity);
        manager.decrease(1);
        manager.setTimeToIncrease(3000);
    }
});
