...
export class GuideSkintManager {
    async openFormDDUI(player, skintIdx) {
        ...
        let idx = 0;
        const filtered = [];
        const filteredObservable = [];
        const searchObs = ui.Observable.create("", {
            clientWritable: true,
        });
        form.textField("Поиск", searchObs).spacer();
        for (const skint of skints) {
            const block = player.dimension.getBlock(skint.location);
            const cost = Math.round(Vector3Utils.distance(player.location, block.center()) / 15);
            const obs = ui.Observable.create(true);
            filtered.push(skint);
            filteredObservable.push({
                name: skint.name,
                obs: obs,
            });
            form.button(cost < 1 ? `${skint.name} (вы тут)` : `${skint.name} (${cost} спарков)`, () => {
                const sparkManager = new SparkManager(player);
                if (sparkManager.get() < cost) {
                    player.sendMessage(`[!] Недостаточно Спарков для перемещения. Требуется еще ${cost - sparkManager.get()} Спарков.`);
                    uiManager.set(false);
                    return;
                }
                player.camera.fade({
                ...
                player.dimension.spawnParticle(namespace.namespaced("guide_skint_sparkles"), block.center());
                uiManager.set(false);
                form.close();
            }, {
                visible: obs,
    ...
    openForm(player, skintIdx) {
        ...
        form.show(player).then((val) => {
            ...
            if (val.selection == 2) {
                this.openForm(player, skints[skintIdx + 1] == undefined ? 0 : skintIdx + 1);
            }
            else if (val.selection == 0) {
                this.openForm(player, skints[skintIdx - 1] == undefined ? skints.length - 1 : skintIdx - 1);
            }
            else {
                const sparkManager = new SparkManager(player);
                if (sparkManager.get() < cost) {
