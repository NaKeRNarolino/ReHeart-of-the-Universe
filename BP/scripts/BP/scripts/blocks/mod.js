...
export function registerBlockComponents(data) {
    guide_skint.register(data);
}
mc.world.afterEvents.worldLoad.subscribe(() => {
    mc.system.run(async () => {
        const skints = new MassiveJSONStorage(namespace.namespaced("guide_skint_storage")).access();
        for (let skintR in skints) {
            const skint = skintR;
            console.warn("loading", skint);
            mc.world
                .getDimension("overworld")
                .runCommand(`tickingarea add ${skint.location.x} ${skint.location.y} ${skint.location.z} ${skint.location.x} ${skint.location.y} ${skint.location.z} temp false`);
