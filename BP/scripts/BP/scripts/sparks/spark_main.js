...
//   return curve;
// }
export function summonSparks(dimension, location, amount, radius, onGrabSingle, valueFromOne = 1) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        const spark = dimension.spawnEntity(namespace.namespaced("spark"), summonLocation);
    ...
    mc.system.runTimeout(() => {
        ...
        const run = mc.system.runInterval(() => {
            for (const spark of sparks) {
                ...
                const points = calculatePointsAlongLine(spark.location, targetLocation, Vector3Utils.distance(spark.location, targetLocation) * 10);
                ...
                for (let i = 0; i < points.length; i++) {
                    ...
                    mc.system.runTimeout(() => {
                        ...
                        if (Vector3Utils.distance(Vector3Utils.add(player.location, {
                            x: 0,
                            y: 1.15,
                            z: 0,
                        }), spark.location) < 0.05) {
...
mc.system.afterEvents.scriptEventReceive.subscribe((data) => {
    if (data.id == "spark:spark") {
        summonSparks(data.sourceEntity.dimension, data.sourceEntity.location, 100, 3, (v) => {
