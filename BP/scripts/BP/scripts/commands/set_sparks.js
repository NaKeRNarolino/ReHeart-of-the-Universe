...
mc.world.beforeEvents.chatSend.subscribe((data) => {
    if (data.message.startsWith("@setsparks")) {
        ...
        if (data.sender.playerPermissionLevel != mc.PlayerPermissionLevel.Operator) {
            data.sender.sendMessage("[!] Использовать команду @setsparks могут только операторы.");
            return;
        }
...
function sparksCommand(origin, players, action, amount) {
    if (action == "set") {
        if (amount == undefined) {
            return {
...
export function register(registry) {
    ...
    registry.registerCommand({
        ...
        description: "Управление количеством спарков у игрока. Для операции `set` параметр `amount` обязателен.",
        mandatoryParameters: [
            {
